<div align="center">

# Portfolio Projects Documentation

### Public documentation for my full-stack web applications, frontend projects, and AI-powered workflows

This repository contains professional documentation for selected projects I have built, including project overviews, live demos, tech stacks, features, architecture notes, workflow explanations, and development highlights.

<br />

[![Portfolio](https://img.shields.io/badge/Portfolio-Live%20Website-brightgreen?style=for-the-badge)](https://profaile-19e99.web.app/)
[![GitHub](https://img.shields.io/badge/GitHub-mohammadbzoor-181717?style=for-the-badge&logo=github)](https://github.com/mohammadbzoor)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mohammed%20AL%20Bzoor-0A66C2?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/mohammadbzoor)

</div>

---

## Projects — Ranked by Complexity & Scope

Projects are grouped below from most to least technically advanced, based on architecture, backend depth, and AI/automation integration.

### 🏆 Tier 1 — AI & Automation Systems

| Project | Type | Tech Stack | Live Demo | Documentation |
|---|---|---|---|---|
| **TechNetwork** | AI-Powered Recruitment Platform | React.js, n8n, OpenAI, Pinecone, Cohere | [AI Workflows](https://github.com/mohammadbzoor/n8n-ai-resume-analyzer) | [View Docs](./technetwork/README.md) |
| **Alpha** | Fintech Backend & AI Automation | Node.js, Express, MySQL, n8n, OpenAI | [Repository](https://github.com/mohammadbzoor/AlphaAPP) | [View Docs](./ALPHA/README.md) |
| **AIrRoute Smart Flight Platform** | Hackathon / Smart Travel Platform | React.js, Bootstrap, Gemini, Custom Dataset | Coming Soon | [View Docs](./airroute-smart-flight-platform/README.md) |

### 🥈 Tier 2 — Full Backend & SaaS Systems

| Project | Type | Tech Stack | Live Demo | Documentation |
|---|---|---|---|---|
| **E-Commerce REST API** | E-Commerce Backend API | Node.js, Express, MongoDB, JWT, Stripe | [Repository](https://github.com/mohammadbzoor/shopBackend) | [View Docs](./ecommerce-backend-api/README.md) |
| **Tesla Coffee** | Cafe / Restaurant POS & Management SaaS | React.js, Firebase, Framer Motion | [Repository](https://github.com/mohammadbzoor/TeslaCoffee) | [View Docs](./Tesla%20coffee%20management%20system/README.md) |
| **Restaurant Management System** | Restaurant / Ordering Web App | React.js, Bootstrap, Firebase | [Live Demo](https://projectm-f5389.web.app/) | [View Docs](./restaurant-management-system/README.md) |

### 🥉 Tier 3 — Full-Stack Web Applications

| Project | Type | Tech Stack | Live Demo | Documentation |
|---|---|---|---|---|
| **StoreReact E-Commerce App** | E-Commerce Web App | React.js, Firebase, SCSS | [Live Demo](https://mohammadbzoor-9490e.firebaseapp.com/) | [View Docs](./store-react-ecommerce/README.md) |
| **Time4Study Study Planner** | Productivity / Study Planning Web App | React.js, Vite, Firebase, Bootstrap, Tailwind CSS | Coming Soon | [View Docs](./time4study-study-planner/README.md) |
| **Online Quiz Platform** | Educational Web App | React.js, Firebase, SCSS | [Live Demo](https://singup-12a91.web.app/) | [View Docs](./online-quiz-platform/README.md) |

### 🎨 Tier 4 — Frontend-Focused Projects

| Project | Type | Tech Stack | Live Demo | Documentation |
|---|---|---|---|---|
| **TBR3 React Frontend** | Charity / Donation Platform Frontend | React.js, Tailwind CSS, React Router | Coming Soon | [View Docs](./tbr3-react-frontend/README.md) |
| **Portfolio Web Application** | Full-Stack Web App | React.js, Firebase | [Live Demo](https://profaile-19e99.web.app/) | [View Docs](./portfolio-web-app/README.md) |

---

## About This Repository

This repository is used to publicly showcase documentation for my projects while keeping some source code repositories private when needed.

It provides recruiters, collaborators, and developers with a clear overview of the projects I have worked on, including the technologies used, core features, live demos, workflow details, and development process behind each project.

Each project documentation may include:

- Project overview
- Main features
- Tech stack
- Live demo links
- Architecture notes
- Key functionalities
- Setup instructions
- Development highlights
- What I learned
- Future improvements

---

## Repository Structure

```bash
Portfolio-Projects-Docs/
│
├── README.md
│
├── technetwork/                          # Tier 1 — AI & Automation
│   ├── README.md
│   ├── frontend-development.md
│   └── ai-pipelines.md
│
├── ALPHA/                                # Tier 1 — AI & Automation
│   └── README.md
│
├── airroute-smart-flight-platform/       # Tier 1 — AI & Automation
│   └── README.md
│
├── ecommerce-backend-api/                # Tier 2 — Backend & SaaS
│   └── README.md
│
├── Tesla coffee management system/       # Tier 2 — Backend & SaaS
│   └── README.md
│
├── restaurant-management-system/         # Tier 2 — Backend & SaaS
│   └── README.md
│
├── store-react-ecommerce/                # Tier 3 — Full-Stack Apps
│   └── README.md
│
├── time4study-study-planner/             # Tier 3 — Full-Stack Apps
│   └── README.md
│
├── online-quiz-platform/                 # Tier 3 — Full-Stack Apps
│   └── README.md
│
├── tbr3-react-frontend/                  # Tier 4 — Frontend-Focused
│   └── README.md
│
└── portfolio-web-app/                    # Tier 4 — Frontend-Focused
    └── README.md
```

---

## Featured Highlights

| Area | Projects |
|---|---|
| AI-Powered Recruitment | TechNetwork |
| Fintech / Backend Systems | Alpha |
| Backend & API Development | Alpha, E-Commerce REST API |
| Hackathon Projects | AIrRoute Smart Flight Platform |
| POS / SaaS Operations Tools | Tesla Coffee |
| E-Commerce & Ordering Systems | StoreReact, Restaurant Management System, Tesla Coffee, E-Commerce REST API |
| Full-Stack Web Applications | Portfolio Web Application, Restaurant Management System, StoreReact, Tesla Coffee |
| Educational Platforms | Online Quiz Platform, Time4Study Study Planner |
| Charity / Donation Platforms | TBR3 React Frontend |
| Firebase-Based Projects | Portfolio Web Application, Restaurant Management System, Online Quiz Platform, StoreReact, Time4Study, Tesla Coffee |
| React Frontend Work | All listed projects |

---

## Purpose

The goal of this repository is to organize my project documentation in one public place and present my work in a professional and accessible way.

Some projects may have private source code, but their documentation is shared publicly to demonstrate the idea, features, technologies, architecture, workflow, and development process behind each project.

---

## Main Technologies

<div align="center">

![React](https://img.shields.io/badge/React.js-61DAFB?style=for-the-badge&logo=react&logoColor=black)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=for-the-badge&logo=bootstrap&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind%20CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![React Router](https://img.shields.io/badge/React%20Router-Routing-CA4245?style=for-the-badge&logo=react-router&logoColor=white)
![SCSS](https://img.shields.io/badge/SCSS-CC6699?style=for-the-badge&logo=sass&logoColor=white)
![n8n](https://img.shields.io/badge/n8n-Automation-EA4B71?style=for-the-badge)
![OpenAI](https://img.shields.io/badge/OpenAI-Integration-412991?style=for-the-badge&logo=openai&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-Chatbot%20Integration-4285F4?style=for-the-badge&logo=google&logoColor=white)

</div>

---

## Author

<div align="center">

### Mohammed AL Bzoor

**Full Stack Developer | React & AI Automation Engineer**

[![GitHub](https://img.shields.io/badge/GitHub-mohammadbzoor-181717?style=for-the-badge&logo=github)](https://github.com/mohammadbzoor)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mohammed%20AL%20Bzoor-0A66C2?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/mohammadbzoor)
[![Portfolio](https://img.shields.io/badge/Portfolio-Live%20Website-brightgreen?style=for-the-badge)](https://profaile-19e99.web.app/)

</div>

---

## Note

This repository is focused on documentation and project presentation.
Some source code repositories may remain private depending on project requirements, privacy, or deployment configuration.
